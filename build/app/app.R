...
#   tagList(singleton(tags$head(includeHTML(("www/gtag.html")))))
# }
...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    reqSignin(
      accordion(
        ...
        accordion_panel(
          ...
          p(
            "1. Select the patient's name then click 'Get Patient History'",
            br(),
      ...
      accordion(
        ...
        accordion_panel(
          ...
          textAreaInput("user_prompt",
            "Prompt:",
            value = config_prompt_user_default,
            height = "300px"
          ),
      ...
      accordion(
        ...
        accordion_panel(
          ...
          selectInput(
            inputId = "model_id_select",
            label = "Model ID:",
            choices = config_model_id,
            selected = config_model_id
          ),
          sliderInput("temperature", "Temperature", min = 0.0, max = 1.0, value = 0.0, step = 0.1),
          ...
          sliderInput("top_p", "Top-P", min = 0, max = 1, value = 0, step = 0.01),
    ...
    tags$div(
      ...
      HTML(
        "Learn More: <br/>
         <a href='https://cloud.google.com/vertex-ai/generative-ai/docs/learn/prompts/introduction-prompt-design' target='_blank'>Introduction to prompting</a> <br/>
...
server <- function(input, output, session) {
  ...
  summary_text <- eventReactive(input$get_summary, {
    ...
    prompt_text <- glue(input$user_prompt)

    # Send the response to the Vertex AI
    response <- gcva_gemini_text(
      projectId = project_id,
      locationId = region,
      prompt = prompt_text,
      modelId = input$model_id_select,
  ...
  output$summary <- renderUI({
    ...
    tryCatch(
      {
        switch(summary_out(), # lookup value
  ...
  webmd_search_ui <- eventReactive(input$get_patient_history, {
    ...
    tags$iframe(
      src = paste0("https://www.webmd.com/search/search_results/default.aspx?query=", query),
      height = 600, width = "100%"
    )
  })

  # render output based on
